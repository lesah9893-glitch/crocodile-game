...
WORD_BANK = {
    "Животные": [
        "крокодил","слон","пингвин","зебра","лев","тигр","медведь","волк","лиса","заяц","белка","ёж","мышь","кот","собака","лошадь","корова","свинья","утка","попугай","орёл","сова","кит","дельфин","акула","медуза","осьминог","черепаха","змея","обезьяна","горилла","панда","гепард","енот"
    ],
    "Спорт": [
        "футбол","хоккей","баскетбол","волейбол","теннис","бокс","плавание","биатлон","лыжи","сноуборд","скейтборд","шахматы","гольф","бег","прыжок","карате","дзюдо","борьба","гимнастика","велоспорт","серфинг","боулинг","бильярд","фигурное катание"
    ],
    "Еда": [
        "пицца","бургер","суши","борщ","пельмени","макароны","картошка","мороженое","шоколад","торт","арбуз","банан","яблоко","сыр","колбаса","бутерброд","шаурма","омлет","салат","суп","блины","пончик","хлеб"
    ],
    "Техника": [
        "компьютер","телефон","ноутбук","телевизор","микрофон","наушники","камера","холодильник","пылесос","утюг","чайник","принтер","роутер","клавиатура","мышка","монитор","колонка","флешка","зарядка","фотоаппарат","проектор"
    ],
    "Транспорт": [
        "машина","автобус","трамвай","троллейбус","поезд","самолёт","вертолёт","корабль","лодка","катер","велосипед","мотоцикл","самокат","метро","такси","трактор","грузовик","ракета","яхта","подводная лодка"
    ],
    "Действие": [
        "бежать","прыгать","танцевать","петь","спать","есть","пить","смеяться","плакать","кричать","читать","писать","рисовать","плавать","лететь","готовить","целовать","обнимать","чихать","зевать","копать","фотографировать","звонить","махать","прятаться"
    ]
}
        ...
        while True:
            ...
            if action == "set_name":
                ...
                if room.leader_id != player_id:
                    await websocket.send_json({
                        "type":"error",
                        "message":"Только ведущий может завершить раунд."
                    })
                    continue
                room.is_active = False
                room.leader_id = None
                ...
                await room.broadcast({
                    "type":"system",
                    "message":"Раунд завершён. Можно выбрать нового ведущего."
                })
                await room.broadcast({"type":"round_stopped"})
                ...
                if room.leader_id == player_id:
                    ...
                    room.remember_message(message_id,message_data)
                ...
                if room.is_active and room.leader_id and room.current_word:
                    if normalized == normalize_word(room.current_word):
                        ...
                        continue
                    proximity = get_proximity(raw_text,room.current_word)
                    message_id = room.create_message_id()
                    message_data = {
                        "type":"chat",
                        "message_id":message_id,
